...
class prod_magento(osv.osv):
	_inherit = 'product.product'
        _columns = {
                     'magento_attr_id':fields.integer('magento attribute id')
                   }
    ...
    def export_products(self, cr, uid):
        ...
        try :
            ...
            try:
               ...
               all_info_products=[]  
            #end
            local_cr = openerp.pooler.get_db(cr.dbname).cursor()
            for prod in products:
                try:
                    if prod.code:
                        product_data = {
...
#            set_export_finished()
    ...
    def export_categories(self, cr, uid):
        ...
        try:
            ...
            lcr = openerp.pooler.get_db(cr.dbname).cursor()  #local cursor ,magneto_id will be update in openerp using this cursor.  
	    
		
	    for category in category_list:
                cat = self.pool.get('product.category').browse(lcr, uid, category)
                try:
                    category_data = {
                            'name': cat.name,  
                    ...
                    if (cat.magento_id == -1) or (not available):
                        ...
                        self.pool.get('product.category').write(lcr, uid, cat.id, {'magento_id': magento_id, 'modified': False})
                    else:
                        if available:
                            #updating category information in magneto 
                            server.call(session, 'category.update', [cat.magento_id, category_data])
                            total_no_of_records+=1
                            self.pool.get('product.category').write(lcr, uid, cat.id, {'modified': False})
                except:
                   pass
            lcr.commit()#all information of category will update in openerp
