        ...
        if 1== 1:
                ...
                for prod in prods:
                    try:
                        ...
                        if (magento_stock[prod.code])>0.0:
                           total_no_of_records+=1
                    except:
                        pass
                    self.pool.get('stock.inventory').prepare_inventory(cr, uid, [inventory_id])    #inventry_obj
                    self.pool.get('stock.inventory').action_done(cr, uid, [inventory_id])
                return total_no_of_records
