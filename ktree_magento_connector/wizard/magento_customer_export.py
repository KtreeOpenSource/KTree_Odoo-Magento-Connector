#This python file is used to call customer export wizard

import openerp.pooler
import os
from openerp.osv import osv, fields

class wizard_magneto_customer_export(osv.osv_memory):
    _name = 'magneto.customers.export'
    _description = 'Export Customers'

    def do_customers_export(self, cr, uid, ids, context=None):

        """ this function will call during synchronization of Customers from Odoo to Magento """

        result=self.pool.get('magento.configuration').export_customers(cr, uid)
        if (result < 0):
            raise osv.except_osv(('Warning'), ('Export failed, please refer to configure file for failure details.'))
        data_obj = self.pool.get('ir.model.data')
        if result >-1:
            text="%s Customers Exported !" %result
...
wizard_magneto_customer_export()
